/*
https://lzkj-isv.isvjcloud.com/wxgame/activity/8530275?activityId=da8a98459401446c92e95a6cbb9cd22c
爆裂豆豆游戏
活动ID环境变量 WXGAME_ACT_ID
[Script]
cron "21 1 * * *" script-path=jd_wxgame.js,tag=通用游戏任务
*/
...
!(async () => {
    ...
    if (!wxgameActivityId) {
        $.log(`没有通用ID任务，尝试获取远程`);
        let data = await getData("https://gitee.com/feverrun/json/raw/master/wxgame.json")
        if (data && data.length) {
            $.log(`获取到远程且有数据`);
