    ...
    for (let i = 0; i < cookiesArr.length && $.needhelp; i++) {
        ...
        if (cookie) {
            ...
            for (let j=cookiesArr.length-1; j>=0; j--) {
                ...
                console.log(`${user} => 帮助 ${userName}`);
                await help(inviteCode);
                await $.wait(2000);
            }

            console.log(`\n`);
            await help('-5o5XeRBqINXAeNedw');
            await $.wait(2000);
        }
    }
...
function help(inviteCode) {
    return new Promise(async (resolve) => {
        let options = taskHelpUrl("party1031_assist", `{"inviteCode":"${inviteCode}"}`)
...
function getInfo() {
    return new Promise(async (resolve) => {
        let options = taskInfoUrl("party1031_init", `{}`)
        $.get(options, async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`);
                    ...
                    data = JSON.parse(data);
                    if (data.data.bizCode ===0 && data.data.result) {
                        inviteCode = data.data.result.inviteCode;
                        if (inviteCode) {
                            console.log(inviteCode);
                            user = $.UserName;
                            inviteCodeArr.push({user:$.UserName, inviteCode:inviteCode});
                        }
                    }
...
function taskInfoUrl(function_id, body) {
    return {
        url: `${JD_API_HOST}/client.action?functionId=${function_id}&advId=${function_id}&body={${body}&client=wh5&clientVersion=1.0.0&appid=o2_act&uuid=cf7d66dca8a794007c133227f504a8e2aff131e7`,
        headers: {
            "Accept": "*/*",
...
function taskHelpUrl(function_id, body) {
    return {
        url: `${JD_API_HOST}/client.action?functionId=${function_id}&advId=${function_id}&client=wh5&clientVersion=1.0.0&appid=o2_act&_stk=appid,body,client,clientVersion,functionId&_ste=1&h5st=&body={"inviteCode":"${inviteCode}"}&uuid=cf7d66dca8a794007c133227f504a8e2aff131e7`,
        headers: {
            "Accept": "*/*",
            "Accept-Encoding": "gzip, deflate, br",
            "Accept-Language": "zh-cn",
