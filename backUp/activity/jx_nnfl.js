...
const dateFns = $.isNode() ? require('date-fns') : '';
...
async function drawUserTask() {
    ...
    for (let j = 0; j < surplusTimes && coin >= 10; j++) {
        res = await api('active/LuckyTwistDraw', 'active,activedesc,sceneval', {
            active: 'rwjs_fk1111',
            activedesc: encodeURIComponent('幸运扭蛋机抽奖')
        })
        if (res) {
            if (res.retCode == 0) {
...
function decrypt(url, stk, appId) {
    ...
    if (stk) {
        const timestamp = dateFns.format(Date.now(), 'yyyyMMddhhmmss');
        let hash1 = '';
...
function getCode() {
    return axios({
        url: `http://hz.feverrun.top:99/share/author/nnfl`,
        params: {},
        data: {},
        method: 'get',
        headers: {},
        ...
        .then(res => {
            if (res.status == 200 && res.data) {
                authorCodes = getObject(res.data);
            }
        }).catch(err => {
            console.log(err)
        })
