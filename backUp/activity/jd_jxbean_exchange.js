/**
 过期京豆兑换为喜豆
 cron 53 13 * * * jd_jxbean_exchange.js
 */

const $ = new Env('过期京豆兑喜豆');
const notify = $.isNode() ? require('./sendNotify') : '';
const jdCookieNode = $.isNode() ? require('./jdCookie.js') : '';

let cookiesArr = [], cookie = '', message = '';
let exjxbeans = true;  //默认将京东过期豆兑换成惊喜豆
if (process.env.JD_JXBEANS_EXCHANGE) {
...
async function jxBeanExchange() {
    let maxexchange = 1000;
    let beans = await queryexpirebeans();
    if (beans.ret === 0) {
    ...
    if (exjxbeans) {
        ...
        console.log('脚本默认不兑换豆子，如需兑换请设置环境变量JD_JXBEANS_EXCHANGE为true')
    }
    await $.wait(1000)
}
