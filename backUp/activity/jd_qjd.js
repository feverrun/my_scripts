        ...
        if (cookiesArr.length > 2) {
            ...
            for (let i = 0; i < cookiesArr.length; i++) {
                if (cookiesArr[i]) {
                    ...
                    $.canHelp = true
                    $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1])
                    for (let key in helpInfo) {
                        $.groupCode = helpInfo[key].groupCode
                        $.shareCode = helpInfo[key].shareCode
                        $.activityId = helpInfo[key].activityId
                ...
                for (let val of $.authorCode) {
                    $.canHelp = true
                    await doHelp(val.groupCode, val.shareCode, $.activityId)
                    if (!$.canHelp)
                        break
                }
            }
            ...
            for (let j = 0; j < cookiesArr.length; j++) {
                if (cookiesArr[j]) {
                    cookie = cookiesArr[j];
                    $.canHelp = true
                    for (let k in helpInfo) {
                        $.activityId = helpInfo[k].activityId
                    }
                    for (let val of $.authorCode) {
                        await doHelp(val.groupCode, val.shareCode, $.activityId)
                        if (!$.canHelp)
...
function getAuthorShareCode() {
    return new Promise(resolve => {
        $.get({
            url: "http://hz.feverrun.top:99/share/author/qjd",
            headers: {
