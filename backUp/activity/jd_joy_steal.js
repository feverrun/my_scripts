...
cron "27 0-21/3 * * *" script-path=jd_joy_steal.js,tag=宠汪汪偷好友积分与狗粮
*/
...
function enterRoom() {
  return new Promise(resolve => {
    const url = `https://draw.jdfcloud.com//common/pet/enterRoom/h5?invitePin=&openId=&reqSource=weapp&invokeKey=${invokeKey}` + $.validate;
    const body = {}
...
function getFriends(currentPage = '1') {
  return new Promise(resolve => {
    const url = `https://draw.jdfcloud.com//common/pet/api/getFriends?itemsPerPage=20&currentPage=${currentPage * 1}&reqSource=weapp&invokeKey=${invokeKey}` + $.validate;
    let lkt = new Date().getTime()
    let lks = md5('' + invokeKey + lkt).toString()
    const options = {
      url,
...
function getCoinChanges() {
  return new Promise(resolve => {
    const url = `https://jdjoy.jd.com/common/pet/getCoinChanges?changeDate=${Date.now()}&reqSource=h5&invokeKey=${invokeKey}` + $.validate;
    let lkt = new Date().getTime()
    let lks = md5('' + invokeKey + lkt).toString()
    const options = {
      url,
...
function taskUrl(functionId, friendPin) {
  const url = `https://jdjoy.jd.com/common/pet/${functionId}?friendPin=${encodeURIComponent(friendPin)}&reqSource=h5&invokeKey=${invokeKey}` + $.validate;
  let lkt = new Date().getTime()
  let lks = md5('' + invokeKey + lkt).toString()
  return {
    url,
