...
!(async () => {
    ...
    if (!($.st <= nowTs && nowTs < $.ed)) {
        ...
        $.http.get({url: `https://raw.githubusercontent.com/feverrun/jd_scripts/main/tools/empty.json`}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
        let hour = (new Date().getUTCHours() + 8) % 24;
        let redIds = await getRedRainIds();
        if (!redIds) redIds = await getRedRainIds('https://raw.githubusercontent.com/feverrun/jd_scripts/main/tools/empty.json');
        $.newAcids = [...(redIds || [])];
