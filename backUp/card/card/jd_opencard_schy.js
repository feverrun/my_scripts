...
3.加购5京豆
  (默认不加购 如需加购请设置环境变量[opencard_addSku114]为"true"
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw114]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw114="3"
填非数字会全都抽奖
...
https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=unionkbblnt20220218dzlhkk&shareUuid=db2a154bbb6d488ea165980c8d1b3c08
)]
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku114 ? process.env.opencard_addSku114 : `${opencard_addSku}`) : ($.getdata('opencard_addSku114') ? $.getdata('opencard_addSku114') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard114 ? process.env.opencard114 : `${opencard}`) : ($.getdata('opencard114') ? $.getdata('opencard114') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait114 ? process.env.openwait114 : `${openwait}`) : ($.getdata('openwait114') ? $.getdata('openwait114') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw114 ? process.env.opencard_draw114 : opencard_draw) : ($.getdata('opencard_draw114') ? $.getdata('opencard_draw114') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard114]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "db2a154bbb6d488ea165980c8d1b3c08"
    ...
    try {
        ...
        if(!$.addCart && !$.outFlag){
            if(opencard_addSku+"" == "true"){
                ...
                console.log('如需加购请设置环境变量[opencard_addSku114]为"true"');
            }
        }
        if(flag){
            await takePostRequest('activityContent');
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw114]为"3" 3为次数');
