/*
1.17～2.15 品质女装 年终狂欢 [jd_opencard112.js]
新增开卡脚本
一次性脚本
...
3.加购5京豆
  (默认不加购 如需加购请设置环境变量[opencard_addSku112]为"true"
4.浏览店铺1豆/个
5.浏览商品1豆/个
...
All变量适用
————————————————
入口：[ 1.17～2.15 品质女装 年终狂欢 (
https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity?activityId=67ba2ad44b8041c9925bf0e72febbdb2&shareUuid=5bbf0cd1dc054167874d6ff75772c546
)]
...
[task_local]
#1.17～2.15 品质女装 年终狂欢
21 0 1-15,10-28 1,2 * jd_opencard112.js, tag=1.17～2.15 品质女装 年终狂欢, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku112 ? process.env.opencard_addSku112 : `${opencard_addSku}`) : ($.getdata('opencard_addSku112') ? $.getdata('opencard_addSku112') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard112 ? process.env.opencard112 : `${opencard}`) : ($.getdata('opencard112') ? $.getdata('opencard112') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait112 ? process.env.openwait112 : `${openwait}`) : ($.getdata('openwait112') ? $.getdata('openwait112') : `${openwait}`);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard112]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "5bbf0cd1dc054167874d6ff75772c546"
    ...
    try {
        ...
        if(!$.addSku && !$.outFlag){
            if(opencard_addSku+"" == "true"){
                ...
                console.log('如需加购请设置环境变量[opencard_addSku112]为"true"');
            }
        }
        $.runFalag = true
        $.log("浏览店铺: " + $.toShop)
