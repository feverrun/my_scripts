...
3.加购5京豆
  (默认不加购 如需加购请设置环境变量[opencard_addSku137]为"true"
4.浏览店铺1豆/个
5.浏览商品1豆/个
...
#4.17~4.23 我的新家，超级配
13 1 23 4 * jd_opencard_wdxj.js, tag=4.17~4.23 我的新家，超级配, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku137 ? process.env.opencard_addSku137 : `${opencard_addSku}`) : ($.getdata('opencard_addSku137') ? $.getdata('opencard_addSku137') : `${opencard_addSku}`)
...
opencard = $.isNode() ? (process.env.opencard137 ? process.env.opencard137 : `${opencard}`) : ($.getdata('opencard137') ? $.getdata('opencard137') : `${opencard}`)
...
openwait = $.isNode() ? (process.env.openwait137 ? process.env.openwait137 : `${openwait}`) : ($.getdata('openwait137') ? $.getdata('openwait137') : `${openwait}`)
...
!(async () => {
    if ($.isNode()) {
        if(opencard+'' != 'true'){
            console.log('如需执行脚本请设置环境变量[opencard137]为"true"')
        }
        if(opencard+'' != 'true'){
            return
        }
    ...
    try {
        ...
        if(!$.addSku && !$.outFlag){
            if(opencard_addSku+'' == 'true'){
                ...
                console.log('如需加购请设置环境变量[opencard_addSku137]为"true"')
            }
        }
        $.runFalag = true
        $.log('浏览店铺: ' + $.toShop)
