/*
12.10～12.21 京选大牌 超会宠你 [jd_opencard82.js]
新增开卡脚本
一次性脚本
...
3.关注10豆
4.加购5京豆
  (默认不加购 如需加购请设置环境变量[opencard_addSku82]为"true"
5.浏览店铺1豆/个
6.浏览商品1豆/个
...
All变量适用
————————————————
入口：[ 12.10～12.21 京选大牌 超会宠你 (https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity?activityId=5870cf9ea76046b4bfc5493f82784eef&shareUuid=cec7c3fd134e435a8fc9fa07650a7755)]
...
31 0,9 10-21 12 * jd_opencard82.js, tag=12.10～12.21 京选大牌 超会宠你, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku82 ? process.env.opencard_addSku82 : `${opencard_addSku}`) : ($.getdata('opencard_addSku82') ? $.getdata('opencard_addSku82') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard82 ? process.env.opencard82 : `${opencard}`) : ($.getdata('opencard82') ? $.getdata('opencard82') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait82 ? process.env.openwait82 : `${openwait}`) : ($.getdata('openwait82') ? $.getdata('openwait82') : `${openwait}`);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard82]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "cec7c3fd134e435a8fc9fa07650a7755"
    ...
    try {
        ...
        if(!$.addSku && !$.outFlag){
            if(opencard_addSku+"" == "true"){
                ...
                console.log('如需加购请设置环境变量[opencard_addSku82]为"true"');
            }
        }
        $.runFalag = true
        $.log("浏览店铺: " + $.toShop)
