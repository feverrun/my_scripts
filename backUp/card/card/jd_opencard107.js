/*
1.14~1.20 大牌会员日 [jd_opencard107.js]
新增开卡脚本
一次性脚本
...
All变量适用
————————————————
入口：[ 1.14~1.20 大牌会员日 (https://lzdz1-isv.isvjcloud.com/dingzhi/bigname/memberday/activity?activityId=dz2201100008884901&shareUuid=86d9ed59cb77482ba76baa1bf295c3be)]
...
[task_local]
#1.14~1.20 大牌会员日
25 0,10 16-21 1 * jd_opencard107.js, tag=1.14~1.20 大牌会员日, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku107 ? process.env.opencard_addSku107 : `${opencard_addSku}`) : ($.getdata('opencard_addSku107') ? $.getdata('opencard_addSku107') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard107 ? process.env.opencard107 : `${opencard}`) : ($.getdata('opencard107') ? $.getdata('opencard107') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait107 ? process.env.openwait107 : `${openwait}`) : ($.getdata('openwait107') ? $.getdata('openwait107') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw107 ? process.env.opencard_draw107 : opencard_draw) : ($.getdata('opencard_draw107') ? $.getdata('opencard_draw107') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard107]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "86d9ed59cb77482ba76baa1bf295c3be"
...
async function run() {
    try {
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw107]为"3" 3为次数');
