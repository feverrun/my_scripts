...
3.加购5京豆
  (默认不加购 如需加购请设置环境变量[opencard_addSku136]为"true"
4.浏览店铺1豆/个
5.浏览商品1豆/个
...
All变量适用
————————————————
入口：[ 4.18~4.24 发现精彩世界，品味趣味人生 (
https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity?activityId=dz690f282607b5c03ea329yjkk2cwl
&shareUuid=c7fccadf10784153bc28d26f13892334
)]
...
3 3 18-24 4 * jd_opencard_jcsj.js, tag=4.18~4.24 发现精彩世界，品味趣味人生, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku136 ? process.env.opencard_addSku136 : `${opencard_addSku}`) : ($.getdata('opencard_addSku136') ? $.getdata('opencard_addSku136') : `${opencard_addSku}`)
...
opencard = $.isNode() ? (process.env.opencard136 ? process.env.opencard136 : `${opencard}`) : ($.getdata('opencard136') ? $.getdata('opencard136') : `${opencard}`)
...
openwait = $.isNode() ? (process.env.openwait136 ? process.env.openwait136 : `${openwait}`) : ($.getdata('openwait136') ? $.getdata('openwait136') : `${openwait}`)
...
!(async () => {
    if ($.isNode()) {
        if(opencard+'' != 'true'){
            console.log('如需执行脚本请设置环境变量[opencard136]为"true"')
        }
        if(opencard+'' != 'true'){
            return
        }
    ...
    try {
        ...
        if(!$.addSku && !$.outFlag){
            if(opencard_addSku+'' == 'true'){
                ...
                console.log('如需加购请设置环境变量[opencard_addSku136]为"true"')
            }
        }
        $.runFalag = true
        $.log('浏览店铺: ' + $.toShop)
