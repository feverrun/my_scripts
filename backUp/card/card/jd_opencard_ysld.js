...
————————————————
入口：[ 雅诗兰黛奢雅邀请入会有礼 (
https://lzkjdz-isv.isvjcloud.com/esteelauder/inviteNew/activityPage?activityId=2203100037674501
&inviterUuid=24f3593f782c4bacb42d3977e0ccbb31)]
请求太频繁会被黑ip
过10分钟再执行

[task_local]
#雅诗兰黛奢雅邀请入会有礼
8 8 1-20 5 * jd_opencard_ysld.js, tag=雅诗兰黛奢雅邀请入会有礼, enabled=true
...
opencard = $.isNode() ? (process.env.opencard120 ? process.env.opencard120 : `${opencard}`) : ($.getdata('opencard120') ? $.getdata('opencard120') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait120 ? process.env.openwait120 : `${openwait}`) : ($.getdata('openwait120') ? $.getdata('openwait120') : `${openwait}`);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard120]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "24f3593f782c4bacb42d3977e0ccbb31"
