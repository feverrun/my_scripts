/*
1.21～1.27 南北年货 新意回家 [jd_opencard111.js]
新增开卡脚本
一次性脚本

1.邀请一人10豆
2.开8张卡 成功开1张 有机会获得100豆
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw111]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw111="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 1.21～1.27 南北年货 新意回家 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzlhkkpoutnkskl20220121&shareUuid=f9f8c978b07b46f480edd77eaf290f20)]
...
[task_local]
#1.21～1.27 南北年货 新意回家
3 11 21-27 1 * jd_opencard111.js, tag=1.21～1.27 南北年货 新意回家, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku111 ? process.env.opencard_addSku111 : `${opencard_addSku}`) : ($.getdata('opencard_addSku111') ? $.getdata('opencard_addSku111') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard111 ? process.env.opencard111 : `${opencard}`) : ($.getdata('opencard111') ? $.getdata('opencard111') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait111 ? process.env.openwait111 : `${openwait}`) : ($.getdata('openwait111') ? $.getdata('openwait111') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw111 ? process.env.opencard_draw111 : opencard_draw) : ($.getdata('opencard_draw111') ? $.getdata('opencard_draw111') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard111]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "f9f8c978b07b46f480edd77eaf290f20"
...
async function run() {
    try {
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw111]为"3" 3为次数');
