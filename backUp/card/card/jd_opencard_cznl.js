...
1.邀请一人10豆
2.开2张卡 成功开1张 获得10豆
3.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw133]为"3"
...
https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzuniongame2022c8e0904b0509d0
...
[task_local]
#4.15~4.25 充值能量 魅力加满
17 22 26 4 * jd_opencard_cznl.js, tag=4.15~4.25 充值能量 魅力加满, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku133 ? process.env.opencard_addSku133 : `${opencard_addSku}`) : ($.getdata('opencard_addSku133') ? $.getdata('opencard_addSku133') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard133 ? process.env.opencard133 : `${opencard}`) : ($.getdata('opencard133') ? $.getdata('opencard133') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait133 ? process.env.openwait133 : `${openwait}`) : ($.getdata('openwait133') ? $.getdata('openwait133') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw133 ? process.env.opencard_draw133 : opencard_draw) : ($.getdata('opencard_draw133') ? $.getdata('opencard_draw133') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard133]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "bc80d987c820478fbebf7cc20bbac197"
...
async function run() {
    try {
        ...
        if(($.hasEnd === true || Date.now() > $.endTime) && nowTime > new Date('2022/04/27 00:00:00+08:00').getTime()){
            ...
            }else console.log('如需抽奖请设置环境变量[opencard_draw133]为"3" 3为次数');
