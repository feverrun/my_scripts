/*
1.18~1.24 会员联合盛典 年货好物集结 [jd_opencard108.js]
新增开卡脚本
一次性脚本
...
3.关注5京豆
4.加购5京豆
  (默认不加购 如需加购请设置环境变量[opencard_addSku108]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw108]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw108="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 1.18~1.24 会员联合盛典 年货好物集结 (
https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzlhkklblsifkpeq0118&shareUuid=3705c4cbf3bc41bf97754ee98cfd5e64
)]
...
30 1 18-24 1 * jd_opencard108.js, tag=1.18~1.24 会员联合盛典 年货好物集结, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku108 ? process.env.opencard_addSku108 : `${opencard_addSku}`) : ($.getdata('opencard_addSku108') ? $.getdata('opencard_addSku108') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard108 ? process.env.opencard108 : `${opencard}`) : ($.getdata('opencard108') ? $.getdata('opencard108') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait108 ? process.env.openwait108 : `${openwait}`) : ($.getdata('openwait108') ? $.getdata('openwait108') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw108 ? process.env.opencard_draw108 : opencard_draw) : ($.getdata('opencard_draw108') ? $.getdata('opencard_draw108') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard108]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "3705c4cbf3bc41bf97754ee98cfd5e64"
    ...
    try {
        ...
        if(!$.addCart && !$.outFlag){
            if(opencard_addSku+"" == "true"){
                ...
                console.log('如需加购请设置环境变量[opencard_addSku108]为"true"');
            }
        }
        if(flag){
            await takePostRequest('activityContent');
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw108]为"3" 3为次数');
