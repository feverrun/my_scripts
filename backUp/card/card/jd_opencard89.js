/*
12.18~12.30 京选大牌 超会宠你 [jd_opencard89.js]
新增开卡脚本
一次性脚本
...
3.加购5京豆
  (默认不加购 如需加购请设置环境变量[opencard_addSku89]为"true"
4.浏览店铺1豆/个
5.浏览商品1豆/个
...
All变量适用
————————————————
入口：[ 12.18~12.30 京选大牌 超会宠你 (https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity?activityId=4c1162ed00874ba5ac0447c30cc45f8a&shareUuid=9b2ed8d774b94911b56673329e36a406)]
...
2 0,12 18-30 12 * jd_opencard89.js, tag=12.18~12.30 京选大牌 超会宠你, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku89 ? process.env.opencard_addSku89 : `${opencard_addSku}`) : ($.getdata('opencard_addSku89') ? $.getdata('opencard_addSku89') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard89 ? process.env.opencard89 : `${opencard}`) : ($.getdata('opencard89') ? $.getdata('opencard89') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait89 ? process.env.openwait89 : `${openwait}`) : ($.getdata('openwait89') ? $.getdata('openwait89') : `${openwait}`);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard89]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "9b2ed8d774b94911b56673329e36a406"
    ...
    try {
        ...
        if(!$.addSku && !$.outFlag){
            if(opencard_addSku+"" == "true"){
                ...
                console.log('如需加购请设置环境变量[opencard_addSku89]为"true"');
            }
        }
        $.runFalag = true
        $.log("浏览店铺: " + $.toShop)
