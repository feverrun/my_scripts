/*
12.18~12.24 联合开卡 [jd_opencard88.js]
新增开卡脚本
一次性脚本
...
[task_local]
#12.18~12.24 联合开卡
33 0,8 22-24 12 * jd_opencard88.js, tag=12.18~12.24 联合开卡, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku88 ? process.env.opencard_addSku88 : `${opencard_addSku}`) : ($.getdata('opencard_addSku88') ? $.getdata('opencard_addSku88') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard88 ? process.env.opencard88 : `${opencard}`) : ($.getdata('opencard88') ? $.getdata('opencard88') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait88 ? process.env.openwait88 : `${openwait}`) : ($.getdata('openwait88') ? $.getdata('openwait88') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw88 ? process.env.opencard_draw88 : opencard_draw) : ($.getdata('opencard_draw88') ? $.getdata('opencard_draw88') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard88]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
...
async function run() {
    try {
        ...
        if(opencard_addSku+"" == "true"){
            ...
            console.log('如需加购请设置环境变量[opencard_addSku88]为"true"');
        }
        await takePostRequest('activity_load');
