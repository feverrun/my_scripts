...
3.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw125]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw125="3"
填非数字会全都抽奖
...
#4.01～4.10 米粉节联合活动
5 21 1-10 4 * jd_opencard_mfj.js, tag=4.01～4.10 米粉节联合活动, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku125 ? process.env.opencard_addSku125 : `${opencard_addSku}`) : ($.getdata('opencard_addSku125') ? $.getdata('opencard_addSku125') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard125 ? process.env.opencard125 : `${opencard}`) : ($.getdata('opencard125') ? $.getdata('opencard125') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait125 ? process.env.openwait125 : `${openwait}`) : ($.getdata('openwait125') ? $.getdata('openwait125') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw125 ? process.env.opencard_draw125 : opencard_draw) : ($.getdata('opencard_draw125') ? $.getdata('opencard_draw125') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard125]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "3373a2581e724af4a5aa3d2b6f7e3dae"
...
async function run() {
    try {
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw125]为"3" 3为次数');
