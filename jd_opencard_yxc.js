...
3.加购5京豆
  (默认不加购 如需加购请设置环境变量[opencard_addSku122]为"true"
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw122]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw122="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 3.22~3.31 大牌好礼 迎春季 (
https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzlhkk068d4d0ab8a1256853002f50&shareUuid=202d93717f9b4cfd809ac1f5e9bdf355)]
...
[task_local]
#3.22~3.31 大牌好礼 迎春季
15 12 22-31 3 * jd_opencard_yxc.js, tag=3.22~3.31 大牌好礼 迎春季, enabled=true
*/
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku122 ? process.env.opencard_addSku122 : `${opencard_addSku}`) : ($.getdata('opencard_addSku122') ? $.getdata('opencard_addSku122') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard122 ? process.env.opencard122 : `${opencard}`) : ($.getdata('opencard122') ? $.getdata('opencard122') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait122 ? process.env.openwait122 : `${openwait}`) : ($.getdata('openwait122') ? $.getdata('openwait122') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw122 ? process.env.opencard_draw122 : opencard_draw) : ($.getdata('opencard_draw122') ? $.getdata('opencard_draw122') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard122]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "202d93717f9b4cfd809ac1f5e9bdf355"
    ...
    try {
        ...
        if(!$.addCart && !$.outFlag){
            if(opencard_addSku+"" == "true"){
                ...
                console.log('如需加购请设置环境变量[opencard_addSku122]为"true"');
            }
        }
        if(flag){
            await takePostRequest('activityContent');
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw122]为"3" 3为次数');
