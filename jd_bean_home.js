...
领京豆额外奖励&抢京豆
活动入口：京东APP首页-领京豆
cron "38 6 * * *" script-path=jd_bean_home.js, tag=领京豆额外奖励
*/
...
!(async () => {
  ...
  await $.wait(parseInit(Math.random() * 10000 + 500, 10))
...
async function jdBeanHome() {
  try {
    ...
    // for (let i = 0; i < 3; ++i) {
    //   await doTask2()
    //   await $.wait(parseInit(Math.random() * 1000 + 1000, 10))
    //   if ($.doneState) break
    // }
    do {
      await doTask2()
      await $.wait(parseInit(Math.random() * 2000 + 2000, 10))
    } while (!$.doneState)
    await $.wait(parseInit(Math.random() * 1000 + 1000, 10))
    await award("feeds")
    await $.wait(parseInit(Math.random() * 1000 + 1000, 10))
    // await signBeanGroupStageIndex()
    // await $.wait(parseInit(Math.random() * 1000 + 1000, 10))
    await getTaskList();
    await receiveJd2();
...
function signBeanGroupStageIndex() {
  return new Promise(resolve => {
...
function showMsg() {
  return new Promise(resolve => {
    if (message) $.msg($.name, '', `【京东账号${$.index}】${$.UserName}\n${message}`);
    resolve()
  })
}
