/*
1.8~1.15 大牌联合 宠爱有礼 [jd_opencard100.js]
新增开卡脚本
一次性脚本
...
3.关注5京豆
4.加购5京豆
  (默认不加购 如需加购请设置环境变量[opencard_addSku100]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw100]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw100="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 1.8~1.15 大牌联合 宠爱有礼 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzlhkklblzms20220108&shareUuid=dab0df6051074e9b82b8bfcb35f17bb0)]
...
[task_local]
#1.8~1.15 大牌联合 宠爱有礼
cron "15 0,7 7-16 1 *" jd_opencard100.js, tag=1.8~1.15 大牌联合 宠爱有礼, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku100 ? process.env.opencard_addSku100 : `${opencard_addSku}`) : ($.getdata('opencard_addSku100') ? $.getdata('opencard_addSku100') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard100 ? process.env.opencard100 : `${opencard}`) : ($.getdata('opencard100') ? $.getdata('opencard100') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait100 ? process.env.openwait100 : `${openwait}`) : ($.getdata('openwait100') ? $.getdata('openwait100') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw100 ? process.env.opencard_draw100 : opencard_draw) : ($.getdata('opencard_draw100') ? $.getdata('opencard_draw100') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard100]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "dab0df6051074e9b82b8bfcb35f17bb0"
    ...
    try {
        ...
        if(!$.addCart && !$.outFlag){
            if(opencard_addSku+"" == "true"){
                ...
                console.log('如需加购请设置环境变量[opencard_addSku100]为"true"');
            }
        }
        if(flag){
            await takePostRequest('activityContent');
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw100]为"3" 3为次数');
