...
let cookiesArr = [], cookie = "", allMessage = "", newShareCodes = [], message = "";
let myInviteCode;
...
function readShareCode() {
    return new Promise(async resolve => {
        $.get({
            url: `http://hz.feverrun.top:99/share/get/health`, timeout: 60000}, (err, resp, data) => {
            try {
                if (err) {
...
function submitCode(code, user) {
    return new Promise(async resolve => {
        $.get({url: `http://hz.feverrun.top:99/share/submit/health?code=${code}&user=${user}`, timeout: 10000}, (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
