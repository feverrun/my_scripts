...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    for (let j = 0; j < pIA.length && $.canHelp; j++) {
      console.log(`【${$.UserName}】去助力【${pIA[j].userName}】邀请码：${pIA[j].strUserPin}`);
      if ($.UserName === pIA[j].userName) {
        console.log(`助力失败：不能助力自己`)
        continue
      }
      $.max = false;
      await enrollFriend(pIA[j].strUserPin);
      await $.wait(3000);
      if ($.max) {
        pIA.splice(j, 1)
        j--
...
//助力好友
async function enrollFriend(strPin) {
  await $.wait(2500);
  return new Promise(async resolve => {
    await $.wait(3500);
