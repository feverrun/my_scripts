...
All变量适用
————————————————
入口：[ 12.10~12.25 狂欢日联合盛典 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dd20211210xgdby3zem7vp5y&shareUuid=509922c5f2514cd281a7b025ff7a5a37)]
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku81 ? process.env.opencard_addSku81 : `${opencard_addSku}`) : ($.getdata('opencard_addSku81') ? $.getdata('opencard_addSku81') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard81 ? process.env.opencard81 : `${opencard}`) : ($.getdata('opencard81') ? $.getdata('opencard81') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait81 ? process.env.openwait81 : `${openwait}`) : ($.getdata('openwait81') ? $.getdata('openwait81') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw81 ? process.env.opencard_draw81 : opencard_draw) : ($.getdata('opencard_draw81') ? $.getdata('opencard_draw81') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard81]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "509922c5f2514cd281a7b025ff7a5a37"
...
async function run() {
    try {
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw81]为"3" 3为次数');
