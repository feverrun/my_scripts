...
3.加购5京豆
  (默认不加购 如需加购请设置环境变量[opencard_addSku131]为"true"
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw131]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw131="3"
填非数字会全都抽奖
...
All变量适用
https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzlhkkb93b11eca9cc020017000b7b
请求太频繁会被黑ip
过10分钟再执行

[task_local]
#4.13~4.20 大牌好物 春日嗨购
7 21 13-20 4 * jd_opencard_dphw.js, tag=4.13~4.20 大牌好物 春日嗨购, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku131 ? process.env.opencard_addSku131 : `${opencard_addSku}`) : ($.getdata('opencard_addSku131') ? $.getdata('opencard_addSku131') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard131 ? process.env.opencard131 : `${opencard}`) : ($.getdata('opencard131') ? $.getdata('opencard131') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait131 ? process.env.openwait131 : `${openwait}`) : ($.getdata('openwait131') ? $.getdata('openwait131') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw131 ? process.env.opencard_draw131 : opencard_draw) : ($.getdata('opencard_draw131') ? $.getdata('opencard_draw131') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard131]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "2a8155e7fa894fdcba825ab36ea8c0ed"
    ...
    try {
        ...
        if(!$.addCart && !$.outFlag){
            if(opencard_addSku+"" == "true"){
                ...
                console.log('如需加购请设置环境变量[opencard_addSku131]为"true"');
            }
        }
        if(flag){
            await takePostRequest('activityContent');
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw131]为"3" 3为次数');
