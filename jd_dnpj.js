...
如需加购请设置环境变量[unknownTask_addSku11]为"true"
#电脑配件
5 0,6 * * * jd_dnpj.js
*/
const $ = new Env('电脑配件');
...
let unknownTask_addSku1 = "true"
unknownTask_addSku1 = $.isNode() ? (process.env.unknownTask_addSku11 ? process.env.unknownTask_addSku11 : `${unknownTask_addSku1}`) : ($.getdata('unknownTask_addSku11') ? $.getdata('unknownTask_addSku11') : `${unknownTask_addSku1}`);
unknownTask_addSku1 = $.isNode() ? (process.env.unknownTask_addSku1_All ? process.env.unknownTask_addSku1_All : `${unknownTask_addSku1}`) : ($.getdata('unknownTask_addSku1_All') ? $.getdata('unknownTask_addSku1_All') : `${unknownTask_addSku1}`);
allMessage = ""
message = ""
$.hotFlag = false
$.outFlag = 0
...
async function run() {
    try {
        ...
        if($.list){
            for(let i in $.list || []){
                $.oneTask = $.list[i]
                if(unknownTask_addSku1+"" != "true" && $.oneTask.taskName.indexOf('加购') > -1) console.log('\n如需加购请设置环境变量[unknownTask_addSku11]为"true"\n');
                if(unknownTask_addSku1+"" != "true" && $.oneTask.taskName.indexOf('加购') > -1) continue;
