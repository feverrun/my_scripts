/*
cron "36 6 * * *" jd_speed_sign_Part1.js, tag:京东极速版任务1
*/
...
async function sign() {
    return new Promise(resolve => {
        $.get(taskUrl('speedSign', {
                "kernelPlatform": "RN",
                "activityId": "9WA12jYGulArzWS7vcrwhw",   //"8a8fabf3cccb417f8e691b6774938bc2",
                "noWaitPrize": "false"
            }),
            async (err, resp, data) => {
                try {
...
function invite() {
    let t = +new Date()
    let inviterId = ["QeIexkaFC5uOTXe1H68kGw=="][Math.floor((Math.random() * 1))]
    var headers = {
        'Host': 'api.m.jd.com',
