...
function taskPostUrl(functionId,body) {
    return {
        url: `${JD_API_HOST}`,
        body: `appid=signed_wh5&functionId=${functionId}&body=${encodeURIComponent(JSON.stringify(body))}&client=wh5&clientVersion=1.0.0`,
        headers: {
            'Cookie': cookie,
            'Host': 'api.m.jd.com',
...
function getInviteId() {
    let body = {"lbsCity":"2","realLbsCity":"2824","inviteId":'',"headImg":"","userName":"","taskChannel":"1"}
...
function getInfo(inviteId, flag = false) {
    // let body = {"lbsCity":"2","realLbsCity":"2824","inviteId":inviteId,"headImg":"","userName":"","taskChannel":"1"}
    let body = {"lbsCity":"2","realLbsCity":"2824","inviteId":inviteId,"headImg":"","userName":"","taskChannel":"1","location":"121.33988,31.387025","safeStr":""}
    return new Promise((resolve) => {
        $.post(taskPostUrl("city_getHomeDatav1",body), async (err, resp, data) => {
            try {
                if (err) {
...
function readShareCode() {
    console.log(`开始`)
    return new Promise(async resolve => {
        $.get({url: `http://hz.feverrun.top:99/share/get/city`, 'timeout': 10000}, (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
