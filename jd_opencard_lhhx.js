...
3.加购5京豆
  (默认不加购 如需加购请设置环境变量[opencard_addSku119]为"true"
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw119]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw119="3"
填非数字会全都抽奖
...
3 1,10 17-25 3 * jd_opencard_lhhx.js, tag=3.17~3.25 大牌联合 好物焕新季, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku119 ? process.env.opencard_addSku119 : `${opencard_addSku}`) : ($.getdata('opencard_addSku119') ? $.getdata('opencard_addSku119') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard119 ? process.env.opencard119 : `${opencard}`) : ($.getdata('opencard119') ? $.getdata('opencard119') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait119 ? process.env.openwait119 : `${openwait}`) : ($.getdata('openwait119') ? $.getdata('openwait119') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw119 ? process.env.opencard_draw119 : opencard_draw) : ($.getdata('opencard_draw119') ? $.getdata('opencard_draw119') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard119]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    try {
        ...
        if(!$.addCart && !$.outFlag){
            if(opencard_addSku+"" == "true"){
                ...
                console.log('如需加购请设置环境变量[opencard_addSku119]为"true"');
            }
        }
        if(flag){
            await takePostRequest('activityContent');
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw119]为"3" 3为次数');
