...
3.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw132]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw132="3"
填非数字会全都抽奖
...
https://lzdz1-isv.isvjcloud.com/dingzhi/april/springUnion3/activity?activityId=dz3fb7e0f64f21afa7ff66d3daf15b
...
[task_local]
#4.13~4.29 早鸟焕春 大牌惠聚
cron "13 1 14-29 4 *" jd_opencard_znhc.js, tag=4.13~4.29 早鸟焕春 大牌惠聚, enabled=true
*/
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku132 ? process.env.opencard_addSku132 : `${opencard_addSku}`) : ($.getdata('opencard_addSku132') ? $.getdata('opencard_addSku132') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard132 ? process.env.opencard132 : `${opencard}`) : ($.getdata('opencard132') ? $.getdata('opencard132') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait132 ? process.env.openwait132 : `${openwait}`) : ($.getdata('openwait132') ? $.getdata('openwait132') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw132 ? process.env.opencard_draw132 : opencard_draw) : ($.getdata('opencard_draw132') ? $.getdata('opencard_draw132') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard132]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "bb0e53e514bc4a2fbc9efd580f837397"
...
async function run() {
    try {
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw132]为"3" 3为次数');
