...
import requests, json, time, logging, traceback, os, random, op_notify, datetime, configparser
...
def readConfig():
    try:
        ...
        logging.error('1.请检查是否在目录下建立了op_config.ini')
...
def main(event, context):
    ...
    if users.has_option("dingding", 'dingtalkWebhook'):
        op_notify.sendDing(users.get("dingding", "dingtalkWebhook"), users.get("dingding", "dingtalksecret"))  # 钉钉推送日记
    if users.has_option("telegramBot", 'tgToken'):
        op_notify.sendTg(users.get("telegramBot", "tgToken"), users.get("telegramBot", "tgUserId"),
                      users.get("telegramBot", "tghost"))  # TG机器人推送日记
    if users.has_option("pushplus", 'pushplusToken'):
        op_notify.sendPushplus(users.get("pushplus", "pushplusToken"))  # push+ 推送日记
    if users.has_option("enterpriseWechat", 'id'):
        op_notify.sendWechat(users.get("enterpriseWechat", "id"), users.get("enterpriseWechat", "secret"),
                          users.get("enterpriseWechat", "agentld"))  # 企业微信通知
    if users.has_option("IFTTT", 'apiKey'):
        op_notify.sendIFTTT(users.get("IFTTT", "apiKey"), users.get("IFTTT", "eventName"))  # IFTTT 推送日记
    if users.has_option("Bark", 'Barkkey'):
        op_notify.sendBark(users.get("Bark", "Barkkey"), users.get("Bark", "Barksave"))  # Bark推送助手
