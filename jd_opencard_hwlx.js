...
3.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw129]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw129="3"
填非数字会全都抽奖
...
https://lzdz1-isv.isvjcloud.com/dingzhi/april/springUnion3/activity?activityId=dz40c5c3a14fd59a0c47466d63f0c7

请求太频繁会被黑ip
过10分钟再执行

#4.8~4.22 早鸟派对好物来袭
6 15 8-22 4 * jd_opencard_hwlx.js, tag=4.8~4.22 早鸟派对好物来袭, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku129 ? process.env.opencard_addSku129 : `${opencard_addSku}`) : ($.getdata('opencard_addSku129') ? $.getdata('opencard_addSku129') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard129 ? process.env.opencard129 : `${opencard}`) : ($.getdata('opencard129') ? $.getdata('opencard129') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait129 ? process.env.openwait129 : `${openwait}`) : ($.getdata('openwait129') ? $.getdata('openwait129') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw129 ? process.env.opencard_draw129 : opencard_draw) : ($.getdata('opencard_draw129') ? $.getdata('opencard_draw129') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard129]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    authorCodeList = [
        'bb0e53e514bc4a2fbc9efd580f837397',
    ];
...
async function run() {
    try {
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw129]为"3" 3为次数');
