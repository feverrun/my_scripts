/*
1.8~1.26 集年味卡，享年味盛宴 [jd_opencard101.js]
新增开卡脚本
一次性脚本
...
2.开10张卡 成功开1张 获得5豆
3.关注5豆
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw101]为"3"
...
All变量适用
————————————————
入口：[ 1.8~1.26 集年味卡，享年味盛宴 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=cd20220108tpovk25f5b40s07t7&shareUuid=fd903f4dc7be40bab66c47bc324ed5ee)]
...
[task_local]
#1.8~1.26 集年味卡，享年味盛宴
13 2,9 8-26 1 * jd_opencard101.js, tag=1.8~1.26 集年味卡，享年味盛宴, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku101 ? process.env.opencard_addSku101 : `${opencard_addSku}`) : ($.getdata('opencard_addSku101') ? $.getdata('opencard_addSku101') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard101 ? process.env.opencard101 : `${opencard}`) : ($.getdata('opencard101') ? $.getdata('opencard101') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait101 ? process.env.openwait101 : `${openwait}`) : ($.getdata('openwait101') ? $.getdata('openwait101') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw101 ? process.env.opencard_draw101 : opencard_draw) : ($.getdata('opencard_draw101') ? $.getdata('opencard_draw101') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard101]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "fd903f4dc7be40bab66c47bc324ed5ee"
...
async function run() {
    try {
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw101]为"3" 3为次数');
