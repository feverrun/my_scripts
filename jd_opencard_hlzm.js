...
All变量适用
————————————————
入口：[ 海蓝之谜邀请入会有礼 (https://lzkjdz-isv.isvjcloud.com/m/1000410747/99/2203100041074702/?helpUuid=0bb6ea6c8d1a42b0b821e9f2a27f2ce4)]
...
opencard = $.isNode() ? (process.env.opencard118 ? process.env.opencard118 : `${opencard}`) : ($.getdata('opencard118') ? $.getdata('opencard118') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait118 ? process.env.openwait118 : `${openwait}`) : ($.getdata('openwait118') ? $.getdata('openwait118') : `${openwait}`);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard118]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
