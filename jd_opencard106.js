/*
1.14~1.21 超级年货节 惊喜不打烊 [jd_opencard106.js]
新增开卡脚本
一次性脚本
...
3.关注5京豆
4.加购5京豆
  (默认不加购 如需加购请设置环境变量[opencard_addSku106]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw106]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw106="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 1.14~1.21 超级年货节 惊喜不打烊 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzlhkkkbblnt20220114&shareUuid=6bc9cb2990b64e1b8a3e79017e8b4df2)]
...
2 1,13 14-21 1 * jd_opencard106.js, tag=1.14~1.21 超级年货节 惊喜不打烊, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku106 ? process.env.opencard_addSku106 : `${opencard_addSku}`) : ($.getdata('opencard_addSku106') ? $.getdata('opencard_addSku106') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard106 ? process.env.opencard106 : `${opencard}`) : ($.getdata('opencard106') ? $.getdata('opencard106') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait106 ? process.env.openwait106 : `${openwait}`) : ($.getdata('openwait106') ? $.getdata('openwait106') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw106 ? process.env.opencard_draw106 : opencard_draw) : ($.getdata('opencard_draw106') ? $.getdata('opencard_draw106') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard106]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "6bc9cb2990b64e1b8a3e79017e8b4df2"
    ...
    try {
        ...
        if(!$.addCart && !$.outFlag){
            if(opencard_addSku+"" == "true"){
                ...
                console.log('如需加购请设置环境变量[opencard_addSku106]为"true"');
            }
        }
        if(flag){
            await takePostRequest('activityContent');
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw106]为"3" 3为次数');
