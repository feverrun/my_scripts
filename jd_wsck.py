...
def wskey_getwskey():
    possible_paths = ['/wskey/config/account.json', '/jd/config/account.json']
...
def wskey_subcookie(cookie, token):
...
def main():
    ...
    if os.path.exists("/wskey/config/auth.json"):
    ...
    if envtype in ('ql','ql_latest'):
        ...
        for ws,remark in wslist:
            ...
            if "app_open" in cookie:
                #printf("转换成功:"cookie)
                res = wskey_subcookie(cookie, token)
