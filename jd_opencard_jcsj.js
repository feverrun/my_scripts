...
cron "5 14 18-24 4 *" jd_opencard_wdxj.js
*/
...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            authorCodeList = [
                'c7fccadf10784153bc28d26f13892334',
            ]
        ...
        if ($.secretPin) {
            ...
            if ($.openCardStatus) {
                ...
                for (let i = 0; i < ($.openCardStatus.cardList1.length + $.openCardStatus.cardList2.length); i++) {
                    await task('crm/pageVisit/insertCrmPageVisit', `venderId=1000221763&pageId=dz690f282607b5c03ea329yjkk2cwl&elementId=${encodeURIComponent(`去开卡${i}`)}&pin=${encodeURIComponent($.secretPin)}`, 1)
                    await $.wait(2000)
                }
                t1TaskList = []
