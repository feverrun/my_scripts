...
3.加购5京豆
  (默认不加购 如需加购请设置环境变量[opencard_addSku128]为"true"
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw128]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw128="3"
填非数字会全都抽奖
...
————————————————
入口：[ 4.7~4.14 暖春焕新季 (
https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzlhkkc3c34b18b0bc11ec8e240200&shareUuid=f1e21b6c95da43a28b2df29710082fd5)]
...
[task_local]
#4.7~4.14 暖春焕新季
5 2 7-14 4 * jd_opencard_cnhxj.js, tag=4.7~4.14 暖春焕新季, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku128 ? process.env.opencard_addSku128 : `${opencard_addSku}`) : ($.getdata('opencard_addSku128') ? $.getdata('opencard_addSku128') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard128 ? process.env.opencard128 : `${opencard}`) : ($.getdata('opencard128') ? $.getdata('opencard128') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait128 ? process.env.openwait128 : `${openwait}`) : ($.getdata('openwait128') ? $.getdata('openwait128') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw128 ? process.env.opencard_draw128 : opencard_draw) : ($.getdata('opencard_draw128') ? $.getdata('opencard_draw128') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard128]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "f1e21b6c95da43a28b2df29710082fd5"
    ...
    try {
        ...
        if(!$.addCart && !$.outFlag){
            if(opencard_addSku+"" == "true"){
                ...
                console.log('如需加购请设置环境变量[opencard_addSku128]为"true"');
            }
        }
        if(flag){
            await takePostRequest('activityContent');
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw128]为"3" 3为次数');
