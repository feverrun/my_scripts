...
!(async () => {
  ...
  // await getAuthorShareCode('https://raw.fastgit.org/feverrun/updateTeam/main/shareCodes/jd_zz.json');
  await getRandomCode();
...
async function getRandomCode() {
  await $.http.get({url: `https://raw.fastgit.org/feverrun/updateTeam/master/shareCodes/jd_zz.json`, timeout: 10000}).then(async (resp) => {
    if (resp.statusCode === 200) {
      try {
        let { body } = resp;
        body = JSON.parse(body);
