...
[task_local]
#1.1~2.10 大牌集结 入会有礼
33 3,17 1-31 1,2 * jd_opencard96.js, tag=1.1~2.10 大牌集结 入会有礼, enabled=true
...
opencard = $.isNode() ? (process.env.opencard96 ? process.env.opencard96 : `${opencard}`) : ($.getdata('opencard96') ? $.getdata('opencard96') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait96 ? process.env.openwait96 : `${openwait}`) : ($.getdata('openwait96') ? $.getdata('openwait96') : `${openwait}`);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard96]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
