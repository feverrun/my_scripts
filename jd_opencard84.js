/*
12.17~1.3 双旦寻宝 玩转大富翁 [jd_opencard84.js]
新增开卡脚本
一次性脚本
...
3.关注5豆
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw84]为"3"
...
All变量适用
————————————————
入口：[ 12.17~1.3 双旦寻宝 玩转大富翁 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dfw202112nw2thjcb1rl48x88v&shareUuid=bd9fc88528da4c9baa9bbd1cee38dfb0)]
...
23 0,9 17-31,1-3 12,1 * jd_opencard84.js, tag=12.17~1.3 双旦寻宝 玩转大富翁, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku84 ? process.env.opencard_addSku84 : `${opencard_addSku}`) : ($.getdata('opencard_addSku84') ? $.getdata('opencard_addSku84') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard84 ? process.env.opencard84 : `${opencard}`) : ($.getdata('opencard84') ? $.getdata('opencard84') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait84 ? process.env.openwait84 : `${openwait}`) : ($.getdata('openwait84') ? $.getdata('openwait84') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw84 ? process.env.opencard_draw84 : opencard_draw) : ($.getdata('opencard_draw84') ? $.getdata('opencard_draw84') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard84]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "bd9fc88528da4c9baa9bbd1cee38dfb0"
...
async function run() {
    try {
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw84]为"3" 3为次数');
