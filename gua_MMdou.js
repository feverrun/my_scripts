...
async function takePostRequest(type) {
  ...
  switch (type) {
    ...
    case 'beanTaskList1':
      ...
      myRequest = await getGetRequest(`beanTaskList`, encodeURIComponent(body));
      break;
    case 'beanHomeIconDoTask':
      body = `{"flag":"${$.IconDoTaskFlag}","viewChannel":"AppHome"}`;
      myRequest = await getGetRequest(`beanHomeIconDoTask`, encodeURIComponent(body));
      break;
    case 'beanDoTask':
      body = `{"actionType":${$.actionType},"taskToken":"${$.oneActivityInfo.taskToken}"}`;
      myRequest = await getGetRequest(`beanDoTask`, encodeURIComponent(body));
      break;
    case 'morningGetBean':
      body = `{"fp":"-1","shshshfp":"-1","shshshfpa":"-1","referUrl":"-1","userAgent":"-1","jda":"-1","rnVersion":"3.9"}`;
      myRequest = await getGetRequest(`morningGetBean`, encodeURIComponent(body));
      break;
    default:
      console.log(`错误${type}`);
  }
