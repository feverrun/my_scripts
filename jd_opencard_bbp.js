...
3.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw127]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw127="3"
填非数字会全都抽奖
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku127 ? process.env.opencard_addSku127 : `${opencard_addSku}`) : ($.getdata('opencard_addSku127') ? $.getdata('opencard_addSku127') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard127 ? process.env.opencard127 : `${opencard}`) : ($.getdata('opencard127') ? $.getdata('opencard127') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait127 ? process.env.openwait127 : `${openwait}`) : ($.getdata('openwait127') ? $.getdata('openwait127') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw127 ? process.env.opencard_draw127 : opencard_draw) : ($.getdata('opencard_draw127') ? $.getdata('opencard_draw127') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard127]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "6bd1801dda594b0394c5f86acdb32bf7"
...
async function run() {
    try {
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw127]为"3" 3为次数');
