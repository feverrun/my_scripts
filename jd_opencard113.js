/*
2.11~2.22 遇见爱 遇见你 [jd_opencard113.js]
新增开卡脚本
一次性脚本

1.邀请一人10豆
2.开10张卡 成功开1张 获得10豆
3.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw113]为"3"
4.集卡 (默认不集卡 如需集卡请设置环境变量[opencard_compositeCard]为"true"
5.2月23号自动瓜分
...
All变量适用
————————————————
入口：[ 2.11~2.22 遇见爱 遇见你 (
https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=qrj20220211udn6m4p8pqh7lc2&shareUuid=d11c36de1ecc4166ab355842afb1e40e
)]
...
[task_local]
#2.11~2.22 遇见爱 遇见你
6 2 11-23 2 * jd_opencard113.js, tag=2.11~2.22 遇见爱 遇见你, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku113 ? process.env.opencard_addSku113 : `${opencard_addSku}`) : ($.getdata('opencard_addSku113') ? $.getdata('opencard_addSku113') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard113 ? process.env.opencard113 : `${opencard}`) : ($.getdata('opencard113') ? $.getdata('opencard113') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait113 ? process.env.openwait113 : `${openwait}`) : ($.getdata('openwait113') ? $.getdata('openwait113') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw113 ? process.env.opencard_draw113 : opencard_draw) : ($.getdata('opencard_draw113') ? $.getdata('opencard_draw113') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard113]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "d11c36de1ecc4166ab355842afb1e40e"
...
async function run() {
    try {
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw113]为"3" 3为次数');
