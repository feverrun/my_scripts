/*
12.9~12.15 联合开卡 [jd_opencard80.js]
新增开卡脚本
一次性脚本
...
2.开13张 成功开1张 可能获得5京豆
  全部开完获得1次抽奖
3.关注10京豆 获得1次抽奖
4.加购5京豆 获得1次抽奖(默认不加购 如需加购请设置环境变量[opencard_addSku80]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw80]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw80="3"
填非数字会全都抽奖
...
#12.9~12.15 联合开卡
3 0 9-15 12 * jd_opencard80.js, tag=12.9~12.15 联合开卡, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku80 ? process.env.opencard_addSku80 : `${opencard_addSku}`) : ($.getdata('opencard_addSku80') ? $.getdata('opencard_addSku80') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard80 ? process.env.opencard80 : `${opencard}`) : ($.getdata('opencard80') ? $.getdata('opencard80') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait80 ? process.env.openwait80 : `${openwait}`) : ($.getdata('openwait80') ? $.getdata('openwait80') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw80 ? process.env.opencard_draw80 : opencard_draw) : ($.getdata('opencard_draw80') ? $.getdata('opencard_draw80') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard80]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
...
async function run() {
    try {
        ...
        if(opencard_addSku+"" == "true"){
            ...
            console.log('如需加购请设置环境变量[opencard_addSku80]为"true"');
        }
        await takePostRequest('activity_load');
        if(opencard_draw+"" !== "0"){
            $.runFalag = true
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw80]为"3" 3为次数');
...
async function takePostRequest(type) {
    ...
    switch (type) {
        ...
        case 'missionInviteList':
            ...
            admJson = {"inviteListRequest":{"actId":$.actId,"userId":$.userId,"missionType":"shareAct","inviteType": 1,"buyerNick":($.MixNick || '')}}
