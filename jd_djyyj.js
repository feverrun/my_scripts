...
async function quiz(){
    let answer = Math.round((Math.random()*10))%2 == 0 ? "A" : "B";
    let body = `appid=china-joy&functionId=champion_game_prod&body={"activityDate":"${currentdate}","answerCode":"${answer}","apiMapping":"/api/guessAnswer"}&t=${new Date().getTime()}&loginType=2`
    let MyRequest = PostRequest(body)
...
async function getshareid(){
    let MyRequest = PostRequest(`appid=china-joy&functionId=champion_game_prod&body={"apiMapping":"/api/homePage"}&t=${new Date().getTime()}&loginType=2`)
    return new Promise((resolve) => {
        $.post(MyRequest,async(error, response, data) =>{
            try{
                const result = JSON.parse(data)
...
async function dosupport(shareid){
    let body = `appid=china-joy&functionId=champion_game_prod&body=%7B%22shareId%22%3A%22${shareid}%22%2C%22apiMapping%22%3A%22%2Fapi%2FdoSupport%22%7D&t=${new Date().getTime()}&loginType=2`
    let MyRequest = PostRequest(body)
    return new Promise((resolve) => {
        $.post(MyRequest,async(error, response, data) =>{
            try{
...
async function getlist(){
    let MyRequest = PostRequest(`appid=china-joy&functionId=champion_game_prod&body={"activityDate":${currentdate},"apiMapping":"/api/task/getTaskList"}&t=${new Date().getTime()}&loginType=2`)
    return new Promise((resolve) => {
        $.post(MyRequest,async(error, response, data) =>{
            try{
                let result = JSON.parse(data)
