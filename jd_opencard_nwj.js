...
1.邀请一人10豆
2.开10张卡 成功开1张 获得10豆
3.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw115]为"3"
...
https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=uniongame20220303molpd624p&shareUuid=f97ec982adca4f80bdffc7961cd37cf0
)]
...
[task_local]
#3.3~3.18 约惠女王节 嗨购不停
5 3,12 3-18 3 * jd_opencard_nwj.js, tag=3.3~3.18 约惠女王节 嗨购不停, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku115 ? process.env.opencard_addSku115 : `${opencard_addSku}`) : ($.getdata('opencard_addSku115') ? $.getdata('opencard_addSku115') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard115 ? process.env.opencard115 : `${opencard}`) : ($.getdata('opencard115') ? $.getdata('opencard115') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait115 ? process.env.openwait115 : `${openwait}`) : ($.getdata('openwait115') ? $.getdata('openwait115') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw115 ? process.env.opencard_draw115 : opencard_draw) : ($.getdata('opencard_draw115') ? $.getdata('opencard_draw115') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard115]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
...
async function run() {
    try {
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw115]为"3" 3为次数');
