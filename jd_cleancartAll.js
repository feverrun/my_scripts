...
需要算法支持
默认：不执行 如需要请添加环境变量
jd_cleancart_Run="true"
jd_cleancart_SignUrl="" # 算法url
...
jdSignUrl = $.isNode() ? (process.env.jd_cleancart_SignUrl ? process.env.jd_cleancart_SignUrl : `${jdSignUrl}`) : ($.getdata('jd_cleancart_SignUrl') ? $.getdata('jd_cleancart_SignUrl') : `${jdSignUrl}`);

cleancartRun = $.isNode() ? (process.env.jd_cleancart_Run ? process.env.jd_cleancart_Run : `${cleancartRun}`) : ($.getdata('jd_cleancart_Run') ? $.getdata('jd_cleancart_Run') : `${cleancartRun}`);

cleancartProducts = $.isNode() ? (process.env.jd_cleancart_products ? process.env.jd_cleancart_products : `${cleancartProducts}`) : ($.getdata('jd_cleancart_products') ? $.getdata('jd_cleancart_products') : `${cleancartProducts}`);
...
!(async () => {
    ...
    if(cleancartRun !== 'true'){
        console.log('脚本停止\n请添加环境变量[jd_cleancart_Run]为"true"')
        return
    }
    if(!cleancartProducts){
        console.log('脚本停止\n请添加环境变量[jd_cleancart_products]\n清空商品\n内容规则看脚本文件')
        return
    }
    $.out = false
...
function jdSign(fn,body) {
    ...
    try{
        const fs = require('fs');
        if (fs.existsSync('./jd_encryption_sign.js')) {
            const encryptionSign = require('./jd_encryption_sign');
    ...
    return new Promise((resolve) => {
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                //     "data": {
                //     "fn": "cartClearQuery",
                //         "sign": "body=%7B%22homeWishListUserFlag%22%3A%221%22%2C%22userType%22%3A%220%22%2C%22updateTag%22%3Atrue%2C%22showPlusEntry%22%3A%222%22%2C%22hitNewUIStatus%22%3A%221%22%2C%22cvhv%22%3A%22049591%22%2C%22cartuuid%22%3A%22hjudwgohxzVu96krv/T6Hg%3D%3D%22%2C%22adid%22%3A%22%22%7D&uuid=c02e5fe6e366ef7eb4af97e780fd348293cb8705&client=apple&clientVersion=10.1.4&st=1637229949083&sv=102&sign=036663de940a58f79924c00ede932b5f"
                //     }
                // }
                let res = $.toObj(data,data)
                if(typeof res === 'object' && res){
