/*
1.8~1.27 集年味卡 享年味盛典 [jd_opencard97.js]
新增开卡脚本
一次性脚本
...
2.开10张卡 成功开1张 获得5豆
3.关注5豆
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw97]为"3"
5.集卡 (默认不集卡 如需集卡请设置环境变量[opencard_compositeCard]为"true"
  每次执行脚本集到可以合成1张卡不再集卡
  当第2次执行时会再集卡
...
All变量适用
————————————————
入口：[ 1.8~1.27 集年味卡 享年味盛典 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=cd20220108iu890dmnhimj01ied&shareUuid=c590732f4d0343c6ac271286db77558d)]
...
[task_local]
#1.8~1.27 集年味卡 享年味盛典
12 4 7-28 1 * jd_opencard97.js, tag=1.8~1.27 集年味卡 享年味盛典, enabled=true
...
const $ = new Env('1.7~1.27 集年味卡 享年味盛典');
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku97 ? process.env.opencard_addSku97 : `${opencard_addSku}`) : ($.getdata('opencard_addSku97') ? $.getdata('opencard_addSku97') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard97 ? process.env.opencard97 : `${opencard}`) : ($.getdata('opencard97') ? $.getdata('opencard97') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait97 ? process.env.openwait97 : `${openwait}`) : ($.getdata('openwait97') ? $.getdata('openwait97') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw97 ? process.env.opencard_draw97 : opencard_draw) : ($.getdata('opencard_draw97') ? $.getdata('opencard_draw97') : opencard_draw);
...
$.compositeCard = $.isNode() ? (process.env.opencard_compositeCard ? process.env.opencard_compositeCard : opencard_compositeCard) : ($.getdata('opencard_compositeCard') ? $.getdata('opencard_compositeCard') : opencard_compositeCard);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard97]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "c590732f4d0343c6ac271286db77558d"
...
async function run() {
    try {
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw97]为"3" 3为次数');
