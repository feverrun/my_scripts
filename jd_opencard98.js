/*
1.7~1.26 集“年味卡”，享年货盛宴 [jd_opencard98.js]
新增开卡脚本
一次性脚本
...
2.开10张卡 成功开1张 获得5豆
3.关注5豆
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw98]为"3"
...
All变量适用
————————————————
入口：[ 1.7~1.26 集“年味卡”，享年货盛宴 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=cd20220107vcowv119979tm7p&shareUuid=18d0111ebd4a4346ae74748373214702)]
...
37 4 7-27 1 * jd_opencard98.js, tag=1.7~1.26 集“年味卡”，享年货盛宴, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku98 ? process.env.opencard_addSku98 : `${opencard_addSku}`) : ($.getdata('opencard_addSku98') ? $.getdata('opencard_addSku98') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard98 ? process.env.opencard98 : `${opencard}`) : ($.getdata('opencard98') ? $.getdata('opencard98') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait98 ? process.env.openwait98 : `${openwait}`) : ($.getdata('openwait98') ? $.getdata('openwait98') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw98 ? process.env.opencard_draw98 : opencard_draw) : ($.getdata('opencard_draw98') ? $.getdata('opencard_draw98') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard98]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "18d0111ebd4a4346ae74748373214702"
...
async function run() {
    try {
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw98]为"3" 3为次数');
