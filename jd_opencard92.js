...
All变量适用
————————————————
入口：[ 12.31~1.9 大牌联合 年货盛宴 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzlhkklblzms20211227&shareUuid=a69151555c3f4f40b6eaeb6906e63d43)]
...
opencard = $.isNode() ? (process.env.opencard92 ? process.env.opencard92 : `${opencard}`) : ($.getdata('opencard92') ? $.getdata('opencard92') : `${opencard}`);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard92]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    ...
    $.shareUuid = "a69151555c3f4f40b6eaeb6906e63d43"
