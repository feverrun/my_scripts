/*
12.20～12.26 联合开卡 [jd_opencard91.js]
新增开卡脚本
一次性脚本
...
2.开11张 成功开1张 可能获得5京豆
  全部开完获得1次抽奖
3.关注10京豆 获得1次抽奖
4.加购5京豆 获得1次抽奖(默认不加购 如需加购请设置环境变量[opencard_addSku91]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw91]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw91="3"
填非数字会全都抽奖
...
[task_local]
#12.20～12.26 联合开卡
5 0 20-26 12 * jd_opencard91.js, tag=12.20～12.26 联合开卡, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku91 ? process.env.opencard_addSku91 : `${opencard_addSku}`) : ($.getdata('opencard_addSku91') ? $.getdata('opencard_addSku91') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard91 ? process.env.opencard91 : `${opencard}`) : ($.getdata('opencard91') ? $.getdata('opencard91') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait91 ? process.env.openwait91 : `${openwait}`) : ($.getdata('openwait91') ? $.getdata('openwait91') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw91 ? process.env.opencard_draw91 : opencard_draw) : ($.getdata('opencard_draw91') ? $.getdata('opencard_draw91') : opencard_draw);
...
!(async () => {
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencard91]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
...
async function run() {
    try {
        ...
        if(opencard_addSku+"" == "true"){
            ...
            console.log('如需加购请设置环境变量[opencard_addSku91]为"true"');
        }
        await takePostRequest('activity_load');
        if(opencard_draw+"" !== "0"){
            $.runFalag = true
        ...
        }else console.log('如需抽奖请设置环境变量[opencard_draw91]为"3" 3为次数');
